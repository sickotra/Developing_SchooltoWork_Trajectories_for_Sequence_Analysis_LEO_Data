-- DWP Employment - ACTIVITY STATES 
...
-- [table_name or the defined alias].[field_name]
	...
	-- Select variables from the Employment table 
	Emp.startdate,
	Emp.enddate,
...
INNER JOIN [LEO].[2002237].[LEARNER_AE_ID_TO_EDUKEY_LOOKUP] AEID_EDUKEY
	...
	-- Link to Employment table, alias Emp
INNER JOIN [LEO].[2002237].[LEO_Employment] Emp
	ON AEID_EDUKEY.edukey = Emp.Edukey
...
WHERE 
	...
	(Emp.startdate >= '2011-04-06')


ORDER BY
	SC_Pupil.pupilmatchingrefanonymous_spr11, Emp.startdate ASC;