-- DWP Self Employment - ACTIVITY STATES 
...
-- [table_name or the defined alias].[field_name]
	...
	-- Select variables from the Self Employment table
...
-- Link to Self Assessment table, alias SelfEmp
INNER JOIN [LEO].[2002237].[LEO_Self_Assessment] SelfEmp
	ON AEID_EDUKEY.edukey = SelfEmp.Edukey
...
WHERE 
	...
	-- The School Census 2010/11 school leaver cohort have observed self employment records from 2014/15 tax year at ages 19/20
	((SelfEmp.TaxYear = '201415') 

	-- Select only those who were flagged as self employment in the tax year 
	AND
	(SelfEmp.Self_Employed = '1'))
